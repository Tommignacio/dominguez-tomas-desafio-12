...
//Session
app.use(session({
    store: MongoStore.create({ mongoUrl: config.mongoDB.URL("session") }), //guarda en mongo atlas las sessiones(base de datos con nombre session)
...
try {
    httpServer.listen(PORT, () => {
        ...
        mongoose.connect(config.mongoDB.URL("ecommerce"), config.mongoDB.options)
            .then(() => {
